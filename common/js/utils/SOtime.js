...
const time5 = (sTime,eTime) => {
	var startTime = new Date(sTime.replace(/-/g, "/")).getTime(),
		endTime = new Date(eTime.replace(/-/g, "/")).getTime();
	if(startTime.length==13) startTime = (startTime-0)/1000;
	if(endTime.length==13) endTime = (endTime-0)/1000;
	var lang = 0;
	var interval = Math.ceil((endTime-startTime)/1000);
	// 一分钟以内
	if(interval < 60){
		if(interval<10){
			lang = '0' + interval
